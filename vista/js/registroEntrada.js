...
async function cargarDatos() {
    try {
        const response = await fetch('https://almacen-syatec-3ifx.onrender.com/ingresar/mostrar');
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    if (form) {
        form.addEventListener("submit", async function (event) {
            ...
            try {
                const response = await fetch("https://almacen-syatec-3ifx.onrender.com/ingresar/ingresar", {
                    method: "POST",
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    document.getElementById('formEditarEntrada').addEventListener('submit', async function (event) {
        ...
        try {
            const response = await fetch(`https://almacen-syatec-3ifx.onrender.com/ingresar/actualizar/${editarData.id}`, {
                method: 'PUT',
                headers: {
...
async function eliminar(id) {
    ...
    try {
        const response = await fetch(`https://almacen-syatec-3ifx.onrender.com/ingresar/eliminar/${id}`, {
            method: 'DELETE'
        })
        if (response.ok) {
