...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (form) {
        form.addEventListener('submit', async function (event) {
            ...
            try {
                const response = await fetch('https://almacen-syatec-3ifx.onrender.com/trabajador/insertar', {
                    method: 'POST',
                    headers: {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('formEditar').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch(`https://almacen-syatec-3ifx.onrender.com/trabajador/actualizar/${editarData.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
...
async function eliminar(id) {
    ...
    try {
        const response = await fetch(`https://almacen-syatec-3ifx.onrender.com/trabajador/eliminar/${id}`, {
            method: 'DELETE'
        })
        if (response.ok) {
